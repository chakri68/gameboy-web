...
const EXTENSION_HOSTS = ["marketplace.visualstudio.com", "addons.mozilla.org", "chrome.google.com"];
 ...
 */
export function createLaunchCard(rom: Rom, opts: { fallback?: boolean } = {}): HTMLElement {
  const url = rom.demo!;
  const ext = isExtension(url);
  const ctaLabel = ext ? "▶ GET IT" : "▶ LAUNCH IN NEW TAB";

  const card = el("div", { class: "card", role: "group", "aria-label": `${rom.title} launch card` }, [
